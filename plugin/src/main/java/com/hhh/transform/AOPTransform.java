...
public class AOPTransform extends Transform {
  ...
  private static final String TAG = "AOPTransform:";

  private Project mProject;
  private AOPType mAOPType;

  /**
   * @param project 保存下来备用
   */
  public AOPTransform(Project project, AOPType aopType) {
    mProject = project;
    mAOPType = aopType;
  }
  ...
  public void transform(TransformInvocation transformInvocation) {
    ...
    if (mAOPType == AOPType.JAVASSIST) {
      new JavassistCodeInject().injectCode(directoryInputs, libraryJarInputs, mProject);
    } else if (mAOPType == AOPType.ASM) {
      new ASMCodeInject().injectCode(directoryInputs, libraryJarInputs);
    }
